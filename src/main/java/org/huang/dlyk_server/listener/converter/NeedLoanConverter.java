...
public class NeedLoanConverter implements Converter<Object> {
    // 把Excel中的是否需要贷款转换为数据库中的是否需要贷款id
    @Override
    public Object convertToJavaData(ReadCellData<?> cellData, ExcelContentProperty contentProperty, GlobalConfiguration globalConfiguration) throws Exception {
        // cellData -> 是Excel中的数据，比如"需要"
        String needLoan = cellData.getStringValue();
        List<TDicValue> valueList = (List<TDicValue>) DlykServerApplication.CACHE.get(DicEnum.NEEDLOAN.getTypeCode());
        if (valueList != null) {
            for (TDicValue value : valueList) {
                if (value.getTypeValue().equals(needLoan)) {
                    return value.getId();
                }
            }
        }
